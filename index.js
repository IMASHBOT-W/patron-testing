...
if (pairingCode && !NanoBotz.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : 2348133729715")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please enter your whatsapp number 🚹\nFor example: 2348133729715 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
try{
		if (connection === 'close') {
			...
			if (reason === DisconnectReason.badSession) {
                ...
                console.log(`Unknown DisconnectReason: 
${reason}|${connection}`)
             NanoBotzInd();
            }
		}
...
welcome(iswel, isLeft, NanoBotz, anu)
    ...
    async function getMessage(key){
        ...
        return {
            conversation: "PATRON-MD IS HERE"
        }
    }
...
NanoBotz.sendContact2 = async (jid, kon, desk = "Developer Bot", quoted = '', opts = {}) => {
let list = []
for (let i of kon) {
list.push({
displayName: botname,
